...
namespace Boxy_Core.Mvvm
{
    ...
    public sealed class CancelCommand : ICommand
    {
        ...
        {
            Func<object, Task> wrapped = Wrap(executeAsync);
        ...
        {
            Func<Task> wrapped = Wrap(executeAsync);
            return async () =>
            {
                Cancel();
        ...
        {
            Func<object?, Task> wrapped = Wrap((_, token) => executeAsync(token));
            return () => wrapped(null);
        }

        private sealed class RefCountedCancellationTokenSource(CancelCommand parent)
        {
            /// <summary>
            /// The parent <see cref="CancelCommand"/>.
            /// </summary>
            private readonly CancelCommand _parent = parent;
